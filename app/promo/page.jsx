...
// Tambahkan ini supaya fetch data dilakukan saat runtime, bukan build time
...
async function Promo() {
  ...
  try {
    datas = await getData({ action: "GetPromotions" });
  } catch (error) {
    console.error("Fetch promo data failed:", error);
  }

  if (!datas) {
    return (
      <main className="container py-10 text-center">
        <p>Data promo gagal dimuat. Silakan coba lagi nanti.</p>
      </main>
    );
  }
