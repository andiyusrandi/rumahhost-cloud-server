...
export async function POST(req) {
  try {
    ...
    const productWithAdditionalData = await Promise.all(products.map(async (product) => {
      ...
      if (product.name === 'Cloud SSD Starter') productName = `${productNameArray[0]}-${productNameArray[1]}-${productNameArray[2]}`

      const queryStringWhm = whmQueryString({pkg: `rumahhos_${productName}`}, 'getpkginfo')
      ...
      return {
        ...
        ...{isPopular: product.name !== "Cloud SSD Starter" && product.name !== "Cloud SSD Business" ? true : false},
