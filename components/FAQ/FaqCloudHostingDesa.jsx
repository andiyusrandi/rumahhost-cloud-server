...
import { faqCloudHostingDesaData } from "@/utils/data";
import { FaPlus } from "react-icons/fa";

export default function FaqCloudHostingDesa() {
  const dataLength = faqCloudHostingDesaData.length
  return (
      ...
      <div className="container flex flex-col gap-3">
        <div className="relative">
          ...
          <h3 className="py-3 text-[28px] leading-10 font-semibold md:leading-none md:text[40px]">
            Tanya Jawab (FAQ) Cloud Hosting Desa
          </h3>
        </div>

        <Box>
          <Accordion allowToggle gap="4" display="flex" flexDirection="column">
            {faqCloudHostingDesaData.map((data, index) => {
              return (
                <div key={index}>
                  <AccordionItem>
                    {({ isExpanded }) => (
